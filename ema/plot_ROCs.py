...
def read_data(results_file, model, beta=None):
    ...
    #compute loss
    mse_loss.append(mse_split_loss(X_test_scaled, qcd_prediction.astype(np.float32), beta=beta))
    if model=='VAE':
    ...
    for bsm in bsm_labels:
        ...
        mse_loss.append(mse_split_loss(bsm_target, bsm_prediction.astype(np.float32), beta=beta))
        if model=='VAE':
